...
import com.qyhstech.core.threads.TtlThreadPoolTaskExecutor;
import com.qyhstech.spring.configuration.properties.TaskThreadPoolProperties;
...
@Slf4j
public class QyVirtualTaskExecutorConfiguration implements AsyncConfigurer { //配置类实现AsyncConfigurer接口并重写getAsyncExcutor方法，并返回一个ThreadPoolTaskExevutor,就获得了一个基于线程池的TaskExecutor
    ...
    public ThreadPoolTaskExecutor threadPoolTaskExecutor() {
        ...
        executor.setThreadNamePrefix(taskThreadPoolProperties.getThreadNamePrefix()); //异步方法内部线程名称

        // 增加 TaskDecorator 属性的配置,以支持异步获取Request对象
        //        executor.setTaskDecorator(new QyCustomTaskDecorator());
        //        executor.setTaskDecorator(new QyRequestTaskDecorator());
//        executor.setTaskDecorator(TtlRunnable::get); // 包装为TTL执行类
         ...
         * ThreadPoolExecutor.CallerRunsPolicy：重试添加当前的任务，自动重复调用 execute() 方法，直到成功,主线程直接执行该任务，执行完之后尝试添加下一个任务到线程池中，可以有效降低向线程池内添加任务的速度
         */
    ...
    public Executor getAsyncExecutor() {
        log.info("<<<<<<<<<<<<<<< 【QyCore】加载 AsyncExecutor(TTL增强) 服务 >>>>>>>>>>>>>>>>>>");
    ...
    public AsyncUncaughtExceptionHandler getAsyncUncaughtExceptionHandler() {
        return (ex, method, params) -> log.error("Async线程池执行任务发送未知错误,执行方法：{}", method.getName(), ex);
    }
}