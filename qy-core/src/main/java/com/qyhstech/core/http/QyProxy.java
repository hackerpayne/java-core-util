...
public class QyProxy {
    ...
    public static boolean isValidProxy(String input) {
        return ReUtil.isMatch(QyRegexPatterns.PLAIN_PROXY, input);
    }
    ...
    public static String parseProxy(String html) {
        ...
        String text = ReUtil.replaceAll(html, QyRegexPatterns.HTML_TAG, ":");
        text = text.replace("\r\n", ":").replace("\r", ":").replace("\n", ":");
        text = text.replace(" ", "");
        text = ReUtil.replaceAll(text, "(:{1,})", ":");

        text = ReUtil.get(QyRegexPatterns.PLAIN_PROXY, text, 0);
        return text;
    }
